/**
 * Pruebas unitarias del `Motor_Escenarios` (NestJS) y la siembra de
 * predefinidos. Usan un DOBLE EN MEMORIA del puerto
 * `BibliotecaEscenariosRepositorio`, de modo que la logica pura (versionado,
 * fijacion/copia inmutable, siembra idempotente) se valida SIN base de datos
 * viva ni red. Pruebas en Jest (sin vitest).
 *
...
describe('MotorEscenarios: editar versiona sin mutar versiones previas (Req. 29.5, 29.6)', () => {
    ...
    it('genera una nueva version (version + 1) al editar', async () => {
    ...
    it('no muta la version previa: sigue recuperable e intacta', async () => {
        const v1 = await motor.guardar(DEF_BASE);
...
describe('Escenarios predefinidos y siembra (Req. 29.1, 29.7)', () => {
    ...
    it('siembra los predefinidos en una biblioteca vacia', async () => {
        ...
        expect(sembrados.every((e) => e.esPredefinido && e.version === 1)).toBe(true);
    });
