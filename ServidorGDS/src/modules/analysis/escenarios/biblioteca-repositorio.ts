/**
 * Implementacion Prisma del puerto `BibliotecaEscenariosRepositorio`.
 *
 ...
 * traves del `PrismaService` global (migracion base, tarea 3.5).
 *
 * El versionado se modela creando una NUEVA fila por version: editar nunca
...
/** Forma minima de una fila `Scenario` de Prisma usada por el mapeo. */
...
export function aDominio(row: FilaScenario): EscenarioReutilizable {
    return {
        id: row.id,
        nombre: row.nombre,
...
@Injectable()
export class BibliotecaRepositorioPrisma implements BibliotecaEscenariosRepositorio {
