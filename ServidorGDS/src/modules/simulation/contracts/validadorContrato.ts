/**
 * `Validador_Contrato` - frontera de validacion entre la `Capa_Adquisicion`
 * (`Modulo_Simulacion`) y la `Capa_Analisis`.
 *
 ...
 *
 * Migrado al modulo `simulation` (tarea 3.1). Se expone `ValidadorContratoZod`
 * y la instancia reutilizable `validadorContrato` (para consumidores sin DI y
 * para las PBT de las tareas 3.6/3.7), ademas del provider NestJS
 * `ValidadorContratoService` (frontera estable inyectable).
 *
...
/**
 * Provider NestJS del `Validador_Contrato` (frontera estable inyectable de la
 * `Capa_Analisis`). Delega en `ValidadorContratoZod` con el registrador por
 * defecto; se registra y exporta desde `SimulationModule`.
 */
@Injectable()
export class ValidadorContratoService extends ValidadorContratoZod { }
