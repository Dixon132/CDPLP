 ...
 *
 * Se reutiliza el `Validador_Contrato` migrado al modulo `simulation`
 * (`validadorContrato.ts`) sin mocks, ejecutando la serializacion canonica y la
 * deserializacion validante reales sobre contratos generados con `fast-check`.
 * El generador `contratoNormalizadoArb` cubre los casos limite exigidos por la
 * tarea: contenido no-ASCII (acentos, enie, emojis) y listas vacias
 * (`comments[]`/`hashtags[]`).
 *
 * Runner: Jest (`jest pbt`), minimo 100 iteraciones (`{ numRuns: 100 }`).
 *
 ...
 * metadata versionada. Cubre explicitamente listas vacias (`minLength: 0`) y
 * contenido no-ASCII a traves de `textoArb`/`hashtagArb`.
 */
const contratoNormalizadoArb = (): fc.Arbitrary<ContratoNormalizado> =>
    fc.record({
        ...
        comments: fc.array(comentarioArb(), { minLength: 0, maxLength: 6 }),
        image_description: textoArb(),
        hashtags: fc.array(hashtagArb(), { minLength: 0, maxLength: 6 }),
