/**
 * Pruebas unitarias del `Usuario_Sintetico` persistente.
 *
 * Cubren los helpers puros (serializacion/parseo de intereses y actividad,
 * agregacion de patrones de interaccion, mapeo fila<->dominio) y la logica
 ...
 *
 * El acceso a la BD se reemplaza por un doble en memoria que ejerce la MISMA
 * logica real del servicio (findMany/create), sin red.
 *
...
function crearClienteEnMemoria() {
    ...
    const usuarioSintetico = {
        ...
        findMany: async ({ where }: { where: { comunidadId: string } }): Promise<UsuarioRow[]> => {
    ...
    const cliente = { usuarioSintetico, historialUsuario } as unknown as ConstructorParameters<
        typeof ServicioUsuariosSinteticosPrisma
    >[0];
...
describe("actividad (serializacion/parseo)", () => {
    it("round-trip de un registro de actividad", () => {
        ...
        const parsed = parsearActividad(4, serializarActividad(r));
...
describe("mapeo fila<->dominio", () => {
    it("mapUsuarioRowToDominio representa el perfil completo con historial ordenado", () => {
        const rows = [
            historialRow({ id: "h2", numeroSemana: 3, actividad: serializarActividad(registro({ numeroSemana: 3 })) }),
...
// ---------------------------------------------------------------------------
// Servicio: reutilizacion.
// ---------------------------------------------------------------------------
describe("ServicioUsuariosSinteticosPrisma.obtenerOReutilizar", () => {
    it("crea los usuarios en la siembra inicial de la comunidad (Req. 10.1)", async () => {
        ...
        // Cada usuario representa el perfil conductual completo (Req. 10.1).
...
// ---------------------------------------------------------------------------
// Servicio: acumulacion monotonica del historial.
// ---------------------------------------------------------------------------
