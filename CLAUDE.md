...
**Navegación:** fuente única en `ClienteCDPLPL/src/layouts/navigation.js` (`NAV_GROUPS`, `getNavForRole`, `getMobileNavForRole`). La consumen el sidebar, la barra móvil, el buscador Ctrl+K y los breadcrumbs. Debe mantenerse en sync con `MODULOS_POR_ROL` en `Servidor/src/modules/notificaciones/services/index.ts` (visibilidad de notificaciones por rol).
