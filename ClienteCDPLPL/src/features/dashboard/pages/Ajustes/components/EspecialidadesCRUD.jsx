...
function EspecialidadForm({ initial = {}, onSubmit, onCancel, loading }) {
...
const EspecialidadesCRUD = () => {
    ...
    // Modal editar
    const [editTarget, setEditTarget] = useState(null); // especialidad a editar
    ...
    const [toggleTarget, setToggleTarget] = useState(null); // { id, activo }
    ...
    const loadData = async () => {
        setLoadingData(true);
        try {
            const res = await getAllEspecialidadesAdmin();
            setEspecialidades(res.data ?? []);
        } catch {
            showAlert("error", "Error al cargar especialidades.");
        } finally {
        ...
        createEspecialidad(data)
            .then(() => {
                setConfirmSave({
                    ...
                    callback: () => {
                        setShowCreate(false);
                        showAlert("success", "Especialidad creada correctamente.");
                        loadData();
                    },
                });
            })
    ...
    const handleEditSubmit = (data) => {
        ...
        setSavingEdit(true);
        updateEspecialidad(editTarget.id_especialidad, data)
            .then(() => {
                setConfirmSave({
                    open: true,
                    variant: "edit",
                    callback: () => {
                        setEditTarget(null);
                        showAlert("success", "Especialidad actualizada correctamente.");
                        loadData();
                    },
                });
            })
    ...
    const handleToggleEstado = async () => {
        ...
        try {
            await toggleEstadoEspecialidad(toggleTarget.id_especialidad);
            showAlert(
                "success",
                toggleTarget.activo
            ...
            showAlert("error", "Error al cambiar el estado de la especialidad.");
        } finally {
    ...
    return (
        <div className="space-y-4">
            ...
            <ConfirmActionModal
                ...
                message={
                    confirmSave.variant === "create"
                        ? "¿Confirmas que deseas crear esta especialidad?"
            ...
            <ConfirmDeleteModal
                ...
                title={toggleTarget?.activo ? "Desactivar Especialidad" : "Activar Especialidad"}
                message={
                    toggleTarget?.activo
                        ? `¿Confirmas que deseas desactivar la especialidad "${toggleTarget?.nombre}"? No estará disponible en los formularios de colegiados y pasantes.`
                        : `¿Confirmas que deseas activar la especialidad "${toggleTarget?.nombre}"? Volverá a estar disponible en los formularios.`
                }
...
export default EspecialidadesCRUD;
